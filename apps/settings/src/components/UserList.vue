	...
	<div id="app-content" class="user-list-grid" @scroll.passive="onScroll">
		<Modal v-if="showConfig.showNewUserForm" size="small" @close="closeModal">
			<form id="new-user"
				...
				<div v-if="displayFamille"
					style="border: 2px solid black; padding: 5px;">
					<h3><b>{{ t('settings','Info consommation') }}</b></h3>
					<p>
						{{ t('settings','Dans votre forfait, vous avez le droit à :') }}
						<br>
						{{ t('settings','- 5 utilisateurs et 2 To (offre famille)') }}
						<br>
						{{ t('settings','- 6 utilisateurs et 6 To (offre tribu)') }}
						<br>
						{{ t('settings','Au delà, chaque utilisateur supplémentaire vous sera facturé') }}
						<b>{{ t('settings','2€ HT/mois') }}</b>
						{{ t('settings','et chaque To supplémentaire') }}
						<b>{{ t('settings','5€ HT/mois.') }}</b>
					</p>
				</div>
				...
				<div class="groups modal__item">
					...
					<input v-if="!settings.isAdmin"
						id="newgroups"
						:style="displayFamille ? 'display: none;' : ''"
...
export default {
	...
	methods: {
		...
		createUser() {
			...
			let userid = this.newUser.id
			if(this.displayFamille) {
				userid = this.newUser.groups[0].id + "-" + userid
			}
		...
		setNewUserDefaultGroup(value) {
			...
			this.newUser.groups = []
			if (this.displayFamille) {
				let temp = this.groups.filter(group => group.canAdd !== false)
				this.newUser.groups = [ temp[0] ]
			}
		},
